(function($) {
	...
	var methods = {
		...
		init: function(options) {
			...
			function cbFuncNav(cbThis) {
				...
				if (cbThis.attr('data-cb-group') && cbThis.attr('data-cb-group') != '' && cbThis.attr('data-cb-type') != undefined && settings.nav == 'dotted') {
					...
					if (cbGroupTotal > 1) {
						...
						for (var i = 0; i < cbGroupTotal; i++) {
                            ...
                            if ( cbNavType == undefined || cbNavType == 'undefined' ) continue;
    ...
    // перебор изображений 
    $('a:not(.cartonbox) > img').each(function(){
        var $this = $(this), // тег IMG
            $closest = $this.closest('a'), // тег A
            expansion = $closest.attr('href').split( '.' ); // разбитие ссылки на массив
        
        // получение расширения ссылки по последнему элементу массива и перевод в нижний регистр    
        expansion = expansion[ expansion.length - 1 ].toLowerCase();
                
        // проверка получаемого типа
        if ( cartonbox_types[ expansion ] == undefined ) return;
        
        // уставнока необходимых атрибутов
        $closest.attr({
            'data-cb-type': cartonbox_types[ expansion ], //  тип по расширению
            'data-cb-group': 'gallery' // установка галлереи для слайдера            
        });   
        
        // установка класса
        $closest.addClass( 'cartonbox' );          
    });
    
    // проверка количество элементов для cartonbox
    if ( $('.cartonbox').length > 0 ) {
        // запуск cartonbox
        $.cartonbox();
    }
